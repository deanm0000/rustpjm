...
pub async fn df_to_db(df: &DataFrame) {
    ...
    let database_url = format!("postgresql://{}:{}@projectsdb.usspapps.com:55669/projects?sslmode=require", username, password);
    let pool = Pool::<Postgres>::connect(database_url.as_str()).await.unwrap();
    ...
    let cols = match is_da {
        true => vec![col("node"), col("utcbegin"), col("pricedate"), col("hour"),col("dalmp"),col("damcc"),col("damcl")],
        false=> vec![col("node"), col("utcbegin"), col("pricedate"), col("hour"),col("rtlmp"),col("rtmcc"),col("rtmcl")]
    };
